...
const enrichRecommendations = async (mlResponse) => {
  try {
    ...
    console.log(
      "🔍 [ENRICH] Starting enrichment for",
    ...
    if (products.length > 0) {
      ...
      console.log(
        "  - discounts count:",
        sampleProduct.productDiscounts?.length || 0
      );
    }
    ...
    const enriched = recommendations.map((rec, index) => {
      ...
      if (index === 0) {
        console.log(
          "📦 [ENRICH] Sample enriched product:",
