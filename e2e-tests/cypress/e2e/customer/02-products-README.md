...
**Frontend Verification:** ✅ 100% UI Components Verified
...
**Controller:** `backend/src/controllers/publicProduct.controller.js`  
**Routes:** `backend/src/routes/public/product.routes.js`

### ✅ Verified API Endpoints

| Endpoint                   | Method | Purpose                       | Status    |
| -------------------------- | ------ | ----------------------------- | --------- |
| `/api/public/products`     | GET    | Get all products with filters | ✅ Exists |
| `/api/public/products/:id` | GET    | Get product detail            | ✅ Exists |
| `/api/public/categories`   | GET    | Get all categories            | ✅ Exists |
...
```javascript
{
  ...
  search: "keyword",    // ✅ Search by product name (LIKE query)
  category: "cat-id",   // ✅ Filter by category ID
  minPrice: 0,          // ✅ Minimum price filter
  maxPrice: 999999,     // ✅ Maximum price filter
  sortBy: "newest"      // ✅ Sort options (see below)
}
```

### 🔄 Sort Options (Verified in Backend)

| sortBy Value       | SQL Query                     | Status     |
| ------------------ | ----------------------------- | ---------- |
| `newest` (default) | `ORDER BY created_at DESC`    | ✅ Working |
| `name_asc`         | `ORDER BY name ASC`           | ✅ Working |
| `name_desc`        | `ORDER BY name DESC`          | ✅ Working |
| `price_asc`        | `ORDER BY selling_price ASC`  | ✅ Working |
| `price_desc`       | `ORDER BY selling_price DESC` | ✅ Working |

### 💾 Caching Implementation (Verified)

**Cache Strategy:**

- ✅ Products list: 10 minutes TTL (600 seconds)
- ✅ Product detail: 15 minutes TTL (900 seconds)
- ✅ Cache key pattern: `customer:products:{category}:page:{page}`
- ✅ Cache invalidation: On admin product CRUD operations
- ✅ Implementation: `backend/src/cache/cacheService.js`
...
**Page:** `frontend/src/pages/customer/ProductPage.jsx`  
**Components:** `frontend/src/components/ui/ProductCard.jsx`

### ✅ Verified data-cy Selectors

| Element            | data-cy Attribute | Location                | Status    |
| ------------------ | ----------------- | ----------------------- | --------- |
| Search Input       | `search-input`    | ProductPage.jsx:121     | ✅ Exists |
| Category Filter    | `category-filter` | ProductPage.jsx:170     | ✅ Exists |
| Sort Dropdown      | `sort-dropdown`   | ProductPage.jsx:221     | ✅ Exists |
| Product Card       | `product-card`    | ProductCard.jsx:204     | ✅ Exists |
| Product Image      | `product-image`   | ProductImage.jsx:105    | ✅ Exists |
| Product Name       | `product-name`    | ProductCard.jsx:282     | ✅ Exists |
| Product Price      | `product-price`   | ProductCard.jsx:314     | ✅ Exists |
| Add to Cart Button | `add-to-cart-btn` | AddToCartButton.jsx:168 | ✅ Exists |
| Discount Badge     | `discount-badge`  | ProductImage.jsx:156    | ✅ Exists |
| Category Badge     | `category-badge`  | ProductImage.jsx:191    | ✅ Exists |
| Pagination         | `pagination`      | Pagination.jsx:52       | ✅ Exists |
...
```javascript
// Frontend: useDebounce hook (500ms delay)
const debouncedSearch = useDebounce(searchInput, 500);

// Prevents excessive API calls while typing
// User types: "Beras" → Only 1 API call after 500ms pause
```

**Backend Search Logic:**

```sql
-- SQL Query generated by Sequelize
SELECT * FROM products
WHERE name LIKE '%Beras%'
  AND is_active = true
  AND product_type = 'online'
