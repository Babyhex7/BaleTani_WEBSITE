...
export function checkLoginSuccess(response) {
  return check(response, {
    ...
    "login: customer has phone": (r) =>
      r.json("customer.phone_number") !== undefined,
    "login: no password in response": (r) =>
      r.json("customer.password") === undefined,
  });
}
...
export function checkRegisterSuccess(response) {
  return check(response, {
    ...
    "register: no password exposed": (r) =>
      r.json("customer.password") === undefined,
  });
}
...
export function checkProductListSuccess(response) {
  return check(response, {
    ...
    "products: has products array": (r) =>
      Array.isArray(r.json("data.products")),
...
export function checkProductDetailSuccess(response) {
  return check(response, {
    ...
    "product detail: has product_id": (r) =>
      r.json("product.product_id") !== undefined,
...
export function checkErrorResponse(
  ...
  const checks = {
    ...
    "error: has error field": (r) =>
      r.json("error") !== undefined || r.json("message") !== undefined,
  };
...
export function checkCacheHit(response) {
  return check(response, {
    "cache: hit": (r) => {
      const cacheStatus =
        r.headers["X-Cache-Status"] || r.headers["x-cache-status"];
