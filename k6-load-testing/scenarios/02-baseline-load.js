...
export function setup() {
  ...
  if (healthCheck.status !== 200) {
    throw new Error('❌ Backend is not accessible! Check if server is running.');
  }
...
function productBrowsingFlow(baseUrl) {
  group('Product Browsing Flow', function () {
    ...
    let page1Res = http.get(buildUrl(endpoints.public.products, {
      page: 1,
      limit: 12
    }), {
    ...
    // 2. Search products
    let searchRes = http.get(buildUrl(endpoints.public.products, {
      page: 1,
      limit: 12,
    ...
    let filterRes = http.get(buildUrl(endpoints.public.products, {
      page: 1,
      limit: 12,
