...
export default function (data) {
  ...
  let phase;
  if (elapsedSeconds < 300) { // 0-5 min
    phase = 'baseline';
    currentPhase.add(1);
  } else if (elapsedSeconds < 720) { // 5-12 min
    phase = 'spike';
    currentPhase.add(2);
  } else { // 12-20 min
    phase = 'recovery';
...
function aggressiveBehavior(baseUrl, phase) {
  group(`Aggressive Behavior (${phase})`, function () {
    ...
    if (addCartRes.status !== 201 && addCartRes.status !== 200 && addCartRes.status !== 429) {
      errorRate.add(1);
      spikeErrors.add(1);
    }
    ...
    if (Math.random() < 0.3) { // 30% attempt checkout
