...
async function seedTestAccounts() {
  ...
  try {
    ...
    const hashedPassword = await bcrypt.hash(TEST_CONFIG.password, TEST_CONFIG.bcryptRounds);
    ...
    for (let i = 1; i <= TEST_CONFIG.totalAccounts; i++) {
      try {
        ...
        const phoneNumber = TEST_CONFIG.phonePrefix + String(i).padStart(4, '0');
        ...
        if (i % 10 === 0) {
          process.stdout.write(`   Progress: ${i}/${TEST_CONFIG.totalAccounts}\r`);
        }
